...
int main()
{
  ...
  string inver = invert_key(key);
  string encode = encode_letter_subs_ciper(str, key);
  cout << "The decoded string "
       << encode_letter_subs_ciper(encode, inver) << endl;
  return 0;
}
...
/*
 * Function : invert_key
 * Usage : string p = invert_key(key);
 * -----------------------------------------
 *  Returns the decoding key from the encoding key.
 */  
string invert_key(string key)
{
  char arr[26] = {0};
